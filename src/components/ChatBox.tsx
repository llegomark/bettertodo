...
export default function ChatBox({refetch}: Props) {
	...
	// Helper function to count words in the input
	const countWords = input => {
		return input.trim() ? input.trim().split(/\s+/).length : 0
	}

	// Helper function to prevent more than 300 words from being typed
	const handleInputChange = e => {
		const inputText = e.target.value
		if (countWords(inputText) <= 50) setPrompt(inputText)
	}
